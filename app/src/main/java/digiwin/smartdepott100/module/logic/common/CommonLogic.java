...
//    };
     ...
     * 获取扫描明细
     */
    public void getDetail(final ListSumBean sumShowBean, final GetDetailListener listener) {
        ThreadPoolManager.getInstance().executeTask(new Runnable() {
            @Override
            public void run() {
                String createJson = JsonReqForERP.dataCreateJson(mModule, ReqTypeName.GETDETAIL, mTimestamp, sumShowBean);
                OkhttpRequest.getInstance(mContext).post(createJson, new IRequestCallbackImp() {
                    @Override
                    public void onResponse(String string) {
                        String error = mContext.getString(R.string.unknow_error);
